...
parser.add_argument('--list_dir', type=str,
                    default='./lists/ISIC2018', help='list dir')
...
def inference(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        logging.info('idx %d case %s mean_dice %f mean_hd95 %f mean_iou %f mean_precision %f mean_recall %f mean_f1 %f' % (i_batch, case_name, np.mean(metric_i, axis=0)[0], np.mean(metric_i, axis=0)[1], np.mean(metric_i, axis=0)[2], np.mean(metric_i, axis=0)[3], np.mean(metric_i, axis=0)[4], np.mean(metric_i, axis=0)[5]))
    metric_list = metric_list / len(testloader)
    for i in range(1, args.num_classes):
        logging.info('Mean class %d mean_dice %f mean_hd95 %f mean_iou %f mean_precision %f mean_recall %f mean_f1 %f' % (i, metric_list[i-1][0], metric_list[i-1][1], metric_list[i-1][2], metric_list[i-1][3], metric_list[i-1][4], metric_list[i-1][5]))
    ...
    mean_f1 = np.mean(metric_list, axis=0)[5]
    logging.info('Testing performance in best val model: mean_dice : %f mean_hd95 : %f mean_iou : %f mean_precision : %f mean_recall : %f mean_f1 : %f' % (performance, mean_hd95, mean_iou, mean_precision, mean_recall, mean_f1))
